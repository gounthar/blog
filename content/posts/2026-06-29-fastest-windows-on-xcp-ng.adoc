...
If `viridian_time_ref_count`, `viridian_reference_tsc`, `viridian_apic_assist`, `viridian_crash_ctl` or `viridian_stimer` are missing, set them with the VM shut down:
...
I have not benchmarked the before and after, so I am not going to tell you this makes your VM faster. What I can tell you is what the documentation attributes to the missing pieces: a Windows Server 2025 guest with the synthetic timer disabled link:https://docs.xcp-ng.org/troubleshooting/windows-pv-tools/#windows-server-2025-hangs-randomly-with-0-cpu[hangs randomly at 0% CPU], and the standing advice is to check these flags whenever a Windows VM misbehaves on performance or reliability. It costs one command to rule out.
