...
The fix is two lines. Build the VM from the "Other install media" template (so XCP-ng does not impose an OS-specific default), then flip it to UEFI and turn off link:https://en.wikipedia.org/wiki/UEFI#Secure_Boot[Secure Boot], because this pool doesn't have Microsoft's UEFI Secure Boot certificates provisioned (via `secureboot-certs install`), and without them a Secure Boot guest has nothing to validate its signed bootloader against:
